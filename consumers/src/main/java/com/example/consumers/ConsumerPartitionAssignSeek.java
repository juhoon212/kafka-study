...
public class ConsumerPartitionAssignSeek {
    ...
    private static Consumer<String, String> initKafkaConsumer() {
        ...
        props.setProperty(ConsumerConfig.GROUP_ID_CONFIG, "group_pizza_assign_seek_v002");
        ...
        consumer.seek(topicPartition, 10L); // offset 10부터 수신 시작
        return consumer;
    }
    ...
    private static void pollNoCommit(final Consumer<String, String> consumer) {
        try {
            while (true) {
                final ConsumerRecords<String, String> consumerRecords = consumer.poll(Duration.ofMillis(1000));// 1초
                logger.info("####### consumerRecords count:{}", consumerRecords.count());
